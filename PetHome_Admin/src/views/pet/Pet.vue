...
<template>
    <div><!--避免多个根元素-->
        ...
        <el-dialog title="新增/编辑宠物信息" :visible.sync="dialogFormVisible" width="400px">
                ...
                <el-row :gutter="20">
                    <el-col :span="12">
                        <el-form-item label="" label-width="150">
                            ...
                            <p v-model="petForm.adopt.title" autocomplete="off" placeholder="标题"></p>
                        </el-form-item>
                    </el-col>
                    <el-col :span="12">
                        <el-form-item label="" label-width="150">
                            <p v-model="petForm.adopt.name" autocomplete="off" placeholder="萌宠名称"></p>
                        </el-form-item>
                    </el-col>
                </el-row>
                <el-row :gutter="20">
                    <el-col :span="12">
                        <el-form-item label="" label-width="150">
                            <!--双向绑定编辑表单的名称 内显示寻主信息名称进行提示-->
                            <p v-model="petForm.adopt.price" autocomplete="off" placeholder="价格"></p>
                        </el-form-item>
                    </el-col>
                    <el-col :span="12">
                        <el-form-item label="" label-width="150">
                            <p v-model="petForm.adopt.age" autocomplete="off" placeholder="年龄"></p>
                        </el-form-item>
                    </el-col>
                </el-row>
                <el-row :gutter="20">
                    <el-col :span="12">
                        <el-form-item label="" label-width="150">
                            <!--双向绑定编辑表单的名称 内显示寻主信息名称进行提示-->
                            <p v-model="petForm.adopt.sex" autocomplete="off" placeholder="性别"></p>
                        </el-form-item>
                    </el-col>
                    <el-col :span="12">
                        <el-form-item label="" label-width="150">
                            <p v-model="petForm.adopt.address" autocomplete="off" placeholder="地址"></p>
                        </el-form-item>
                    </el-col>
                </el-row>
                <el-row :gutter="20">
                    <el-col :span="12">
                        <el-form-item label="" label-width="150">
                            <!--双向绑定编辑表单的名称 内显示寻主信息名称进行提示-->
                            <p v-model="petForm.adopt.coatColor.name" autocomplete="off" placeholder="毛色"></p>
                        </el-form-item>
                    </el-col>
                    <el-col :span="12">
                        <el-form-item label="" label-width="150">
                            <p v-model="petForm.petType.name" autocomplete="off" placeholder="类型"></p>
                        </el-form-item>
                    </el-col>
                </el-row>
                <el-row :gutter="20">
                    <el-col :span="12">
                        <el-form-item label="" label-width="150">
                            <!--双向绑定编辑表单的名称 内显示寻主信息名称进行提示-->
                            <p v-model="petForm.adopt.user.username" autocomplete="off" placeholder="出售人"></p>
                        </el-form-item>
                    </el-col>
                </el-row>
                <el-row :gutter="20">
        ...
        <el-table
            ...
            <el-table-column prop="name" label="宠物名称" width="180"></el-table-column>
            <el-table-column prop="resources" label="图片资源" width="180"></el-table-column>
            <el-table-column prop="saleprice" label="宠物售价" width="180"></el-table-column>
            <el-table-column prop="offsaletime" label="宠物成本价" width="180"></el-table-column>
            <el-table-column prop="onsaletime" label="上架时间" width="180"></el-table-column>
            <el-table-column prop="costprice" label="宠物成本价" width="180"></el-table-column>
            <el-table-column prop="createtime" label="宠物创建时间" width="180"></el-table-column>
            <el-table-column prop="user.username" label="宠物买主" width="180"></el-table-column>
            ...
            <el-table-column prop="state" label="状态" width="180">
                <template slot-scope="scope">
                    <b v-if="scope.row.state==0" style="color: green">上架</b>
                    <b v-else-if="scope.row.state==2" style="color: green">待激活/上架</b>
                    <b v-else-if="scope.row.state==1" style="color: green">待激活/上架</b>
    ...
    export default {
        ...
        data() {
            return {
                ...
                //直接在挂载完成后直接查询出来 节省资源
                managerOptions: [],//新增修改模态框 选择宠物经理的下拉菜单查询出的所有经理数组
                parentOptions: [],//新增修改模态框 选择上级宠物的下拉菜单查询出的所有宠物数组
        ...
        methods: {
            ...
            handleEdit(id) {
                //编辑宠物
                //发送请求进行数据回显
                this.$http.get("/pet/" + id).then(res => {
                    //获取到的对象直接赋值给编辑表单
                    let {success, msg, data} = res.data;//此时的data用petDetail接收 包括宠物信息
                    if (success) {
                        this.petForm = data;
                        this.fileList = [];
                        let urls = this.petForm.resources.split(",")
            ...
            submitForm() {
                ...
                console.debug("路径" + this.petForm.resources)
                //验证成功后再提交表单 refs参考 新增表单的验证方法-rules
                //进行判断 如果表单的id不为空则是编辑 发送编辑请求
                if (this.petForm.id) {
                    this.$refs.petForm.validate(v => {
                        //如果通过验证再发送请求
                        if (v) {
                            //petForm表单信息双向绑定所以直接作为发送请求的参数
                            //axios发送请求
                            this.$http.post("/pet", this.petForm).then(res => {
            ...
            loadData() {
                ...
                this.$http.patch("/pets", this.searchForm).then(res => {
                ...
                //发送请求 查询所有宠物 因为是宠物组件 必须在每一次做修改也就是加载后再查一遍
            ...
            handleDel(id) {
                ...
                this.$confirm('此操作将永久删除该宠物, 是否继续?', '提示', {
                    ...
                    //发送删除请求
                    this.$http.delete("/pet/" + id).then(res => {
                        //对象解构定义返回对象 新增的data是null
                        let {success, msg, data} = res.data;
                        //判断
                        if (success) {
            ...
            batchOnsale() {
                ...
                this.$confirm('此操作将批量上架, 是否继续?', '提示', {
                    ...
                    //发送删除请求
                    this.$http.get("/onPets/" + ids.join(",")).then(res => {
                        //对象解构定义返回对象 新增的data是null
                        let {success, msg, data} = res.data;
                        //判断
                        if (success) {
            ...
            batchOffsale() {
                ...
                this.$confirm('此操作将批量下架, 是否继续?', '提示', {
                    ...
                    //发送删除请求
                    this.$http.get("/offPets/" + ids.join(",")).then(res => {
                        //对象解构定义返回对象 新增的data是null
                        let {success, msg, data} = res.data;
                        //判断
                        if (success) {
