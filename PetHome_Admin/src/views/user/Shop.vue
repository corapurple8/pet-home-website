<script src="../mock/shop.js"></script>
<template>
    <div><!--避免多个根元素-->
        <!--工具条-->
        <el-col :span="24" class="toolbar" style="padding-bottom: 0px;">
            ...
            <el-form :model="searchForm" :inline="true"><!--双向绑定vue对象的searchForm 以及确定在行内-->
                ...
                <!--搜索框-->
                <el-form-item>
                    <el-input v-model="searchForm.name" placeholder="商铺名称"></el-input>
                </el-form-item>
                <!--搜索确认按钮-->
                <el-form-item>
        ...
        <el-dialog title="新增/编辑商铺信息" :visible.sync="dialogFormVisible" width="400px" >
            <el-form :model="shopForm" :rules="rules" ref="shopForm">
                ...
                <el-form-item label="" label-width="150" prop="name">
                    <!--双向绑定编辑表单的名称 内显示商铺名称进行提示-->
                    <el-input v-model="shopForm.name" autocomplete="off" placeholder="商铺名称" ></el-input>
                </el-form-item>
                <el-form-item label="" label-width="150" prop="sn">
                    <el-input v-model="shopForm.sn" autocomplete="off" placeholder="商铺编号"></el-input>
                </el-form-item>
                ...
                <el-row :gutter="20">
                    <el-col :span="8">
                        <!--选择商铺经理的下拉框-->
                        <el-form-item label="">
                            <!--选择内容双向绑定商铺新增信息表的经理id-->
                            <el-select v-model="shopForm.manager_id" placeholder="请选择商铺经理">
                    ...
                    <el-col :span="8">
                        <!--选择上级商铺的下拉框-->
                        <el-form-item label="">
                            <!--选择内容双向绑定商铺新增信息表的上级商铺parentid-->
                            <el-select v-model="shopForm.parent_id" placeholder="请选择上级商铺">
                                <!--循环添加进去 obj in list label表示显示的文本 value表示在该list数组的值=上级商铺id-->
                                <el-option
                    ...
                    <el-col :span="8">
                        <el-form-item label="" placeholder="请选择状态" v-show="isNeedState">
                            ...
                            <!--单选框-->
                            <!--选择内容双向绑定商铺修改信息表的状态state-->
                            <el-radio v-model="shopForm.state" :label="0">启用</el-radio>
                            <el-radio v-model="shopForm.state" :label="-1">禁用</el-radio>
                        </el-form-item>
                    </el-col>
                </el-row>
            </el-form>
        ...
        <el-table
            ...
            <el-table-column prop="tel" label="商铺电话" width="180"></el-table-column>
            <el-table-column prop="registerTime" label="添加日期" width="180"></el-table-column>
            <el-table-column prop="address" label="商铺地址" width="180"></el-table-column>
            <el-table-column prop="logo" label="商铺logo" width="180"></el-table-column>
            ...
            <el-table-column label="操作" width="150">
                <template scope="scope">
                    ...
                    <el-button type="warning" size="small" @click="shopAudit(scope.row.id)">审核</el-button>
                </template>
            </el-table-column>
        </el-table>
        <!--工具条 批量删除和分页工具栏-->
...
<script>
    ...
    export default {
        name:'shop',
        data() {
            //定义表单验证商铺名称规则 自带参数 rules里的当前内容规则对象 value当前验证的内容值 回调函数
            var deptNameUnique=(rule,value,callback)=>{
                //callback()表示成功
                //callback(new error("失败的提示消息"))
                //发送请求根据商铺name查询
                this.$http.get("/getShopByName/"+value).then(res=>{
                    //res.data是ajaxresult对象 res.data.data返回查询到的商铺
                    let resultId = res.data.data.id;
                    if (resultId){//id存在说明是修改
                        //判断是否和表单本身相同 若是不同则说明修改后和别的商铺重名了
                        if (resultId!=this.shopForm.id){
            ...
            return {
                ...
                //直接在挂载完成后直接查询出来 节省资源
                managerOptions:[],//新增修改模态框 选择商铺经理的下拉菜单查询出的所有经理数组
                parentOptions:[],//新增修改模态框 选择上级商铺的下拉菜单查询出的所有商铺数组
        ...
        methods:{
            ...
            submitForm(){
                ...
                if (this.shopForm.id){
                    this.$refs.shopForm.validate(v=>{
                        //如果通过验证再发送请求
                        if (v){
                            //shopForm表单信息双向绑定所以直接作为发送请求的参数
                            //axios发送请求
                            this.$http.post("/shop",this.shopForm).then(res=>{
                    ...
                    this.$refs.shopForm.validate(v=>{
                        ...
                        if (v){
                            //shopForm表单信息双向绑定所以直接作为发送请求的参数
                            //axios发送请求
                            this.$http.put("/shop",this.shopForm).then(res=>{
                                //对象解构定义返回对象 新增的data是null
                                let {success,msg,data} =res.data;
                                //判断
                                if (success){
            ...
            loadData(){
                ...
                this.$http.patch("/shops",this.searchForm).then(res=>{
                ...
                //发送请求 查询所有商铺 因为是商铺组件 必须在每一次做修改也就是加载后再查一遍
            ...
            batchRemove(){
                ...
                this.$confirm('此操作将永久删除这些商铺, 是否继续?', '提示', {
                    confirmButtonText: '确定',
                    cancelButtonText: '取消',
                    type: 'warning'
                }).then(() => {
                    ...
                    //发送删除请求
                    this.$http.delete("/shops/"+ids.join(",")).then(res=>{
                        //对象解构定义返回对象 新增的data是null
                        let {success,msg,data} =res.data;
                        //判断
                        if (success){
            ...
            shopAudit(id){
                //发送请求
                this.$confirm('此操作会审核通过, 是否继续?', '提示', {type: 'warning'}).then(() => {
                    this.$http.get("/shopAudit/"+id).then(res => {
